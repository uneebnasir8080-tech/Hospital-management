...
<<<<<<< HEAD
  ...
  const { data, isLoading, isFetching, isError } = useQuery({
    queryKey: ["doctorTable", page, limit],
    queryFn: () => getDoctorTable(page, limit),
  ...
  const { mutate: deleteDoctor, isPending: isDeleting } = useMutation({
    ...
    onSuccess: (res) => {
      setDeleteId(null);
      showToast("success", res?.message || "Doctor deleted successfully");
      // Invalidate to refetch fresh data
      queryClient.invalidateQueries({ queryKey: ["doctorTable"] });
    },
    onError: (err) => {
      console.error(err);
  ...
  const filteredResponse = useMemo(() => {
    ...
    return data.getData.filter((item) => {
      ...
      return (
        ...
        item?.doctor?.age?.toString().includes(search) ||
        item?.doctor?.gender?.toLowerCase().includes(search) ||
        item?.doctor?.specialization?.toLowerCase().includes(search) ||
        item?.doctor?.experience?.toLowerCase().includes(search) ||
...
<<<<<<< HEAD
          ...
          {!isLoading && filteredResponse.length === 0 && (
